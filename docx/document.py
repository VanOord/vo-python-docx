...
class Document(ElementProxy):
    ...
    def add_crossreference(self, bookmark, par=None, obj_type="figure"):
        ...
        if bookmark[:3] not in ["fig", "tab", "equ"]:
            str_ = "{:s} is an incorrect bookmark, bookmark should start with fig, tab or equ".format(
                bookmark
            )
    ...
    def bookmark_text(
        self, bookmark_name, text, underline=False, italic=False, bold=False, style=None
    ):
    ...
    def bookmark_picture(
        self, bookmark_name, picture, width=None, height=None, caption="", bmark=""
    ):
        doc_element = self._part._element
        bookmarks_list = doc_element.findall(".//" + qn("w:bookmarkStart"))
        for bookmark in bookmarks_list:
    ...
    def add_bibliography(self):
        ...
        instrText.set(qn("xml:space"), "preserve")  # sets attribute on element
        instrText.text = (
            r"BIBLIOGRAPHY"  # change 1-3 depending on heading levels you need
        )
